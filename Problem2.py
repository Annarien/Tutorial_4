#Problem 2: Take the mcmc sample code. Add a Lorentzian class f(x) = a
#b+(x􀀀c)2 . Run the fit, and show you get correct answers. (10)
...
class Lorentz:
    ...
    # get numerator
    def getLNumerator(self,a):
        lnumerator=a
        return lnumerator

    def getLDenominator (self,b,x,c):
        ldenominator=b+((x-c)**2)
        return ldenominator

    def LorentzianFun(lnumerator,ldenominator):
        lfun=lnumerator/ldenominator
        return lfun
...
if __name__=='__main__':
    ...
    for i in x:
        lnum=Lorentz.getLNumerator(a)
        lden=Lorentz.getLDenominator(b,x,c)
        lfunction=Lorentz.LorentzianFun(lnum,lden)
        print (lfunction)
